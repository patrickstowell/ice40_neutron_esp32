...
namespace NEUTRON {
  ...
  void ConfigureFPGA(){
    ...
    uint16_t DACHVTARGET = (float(CONFIG::HVTARGET) / 3300.) * 4096;
    uint16_t DACHVLIMIT = (float(CONFIG::HVLIMIT) / 3300.) * 4096;
Serial.println("SSetting HV");
Serial.println(DACHVTARGET);

    uint8_t DAC_CONTROL_CONFIG_BYTE0 = ((DACHVTARGET >> 0) & 0xFF);
    uint8_t DAC_CONTROL_CONFIG_BYTE1 = ((DACHVTARGET >> 8) & 0xFF);
    uint8_t DAC_CONTROL_CONFIG_BYTE2 = ((DACHVLIMIT  >> 0) & 0xFF);
    uint8_t DAC_CONTROL_CONFIG_BYTE3 = ((DACHVLIMIT  >> 8) & 0xFF);
